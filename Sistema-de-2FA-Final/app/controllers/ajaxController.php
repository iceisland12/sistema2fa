...
class ajaxController extends Controller {
  ...
  function do_verificar()
  {
    try {
      ...
      if (!$db_token = postModel::has_tokens($user['id'])) {
        throw new Exception('Algo salió mal, intenta más tarde.');
      }

      // Validar el token
      if (!password_verify($token.AUTH_SALT, $db_token['contenido'])) {
        throw new Exception('El código de verificación no es válido o es incorrecto.');
      }

      // Al ser válido generamos un nuevo registro de autorización
      $caducidad = strtotime('+10 minutes'); // tiempo de autorización
      $data      =
      [
        'id_usuario' => $user['id'],
        'tipo'       => '2fa_autorizado',
        'titulo'     => '2FA Autorizado',
      ...
      // Agregar el nuevo registro a la db
      if (!$id_post = postModel::add(postModel::$t1, $data)) {
        throw new Exception('Hubo un problema con tu autorización de acceso, intenta más tarde.');
      }
  ...
  function do_reenviar_codigo()
  {
    try {
      ...
      // Verificar si ya hay autorización válida
      if ($token = postModel::autorizado($user['id'])) {
        throw new Exception('Este dispositivo ya está autorizado.');
      }
